		...
		if err != nil {
			if os.IsNotExist(err) {
				...
				if err := tail.watcher.BlockUntilExists(&tail.Tomb); err != nil {
					...
					return errors.Wrapf(err, "failed to detect creation of %s", tail.Filename)
				}
				continue
			}
			return errors.Wrapf(err, "unable to open file %s", tail.Filename)
		}
		break
	}
	return nil
...
func (tail *Tail) waitForChanges() error {
	...
	case <-tail.changes.Modified:
		return nil
	case <-tail.changes.Renamed:
		tail.changes = nil
		tail.collectRemainder()

		if !tail.ReOpen {
			return ErrStop
		}

		tail.Logger.Printf("Re-opening moved file %s ...", tail.Filename)
	...
	case <-tail.changes.Deleted:
		...
		tail.Logger.Printf("Re-opening deleted file %s ...", tail.Filename)
		if err := tail.reopen(); err != nil {
			return err
		}
		tail.Logger.Printf("Successfully reopened %s", tail.Filename)
...
func (tail *Tail) sendLine(line string) bool {
	...
	if tail.RateLimiter != nil {
		ok := tail.RateLimiter.Pour(uint16(len(lines)))
		if !ok {
...
func (tail *Tail) collectRemainder() {
	time.Sleep(tail.WaitTimeAfterRotation) // wait for remainder
	for {
		line, err := tail.readLine()
